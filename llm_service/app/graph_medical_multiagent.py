...
# Using a smaller local Ollama model by default to fit GPUs with limited VRAM.
# You can change this to another local Ollama model name (e.g., a 2B or 3B
# model) if your GPU has more memory. `gemma3:1b` is a good fast, low-VRAM
# option for concise medical guidance and lists on an RTX 3060 (6 GB).
...
async def assistant_agent(state: MultiAgentState) -> dict:
    ...
    async def _llm_invoke(text: str):
        ...
        try:
            ...
            # Log the error and attempt a simpler invoke. If that also fails,
            # return a friendly fallback so the app doesn't crash and the user
            # receives a helpful message.
            print(f"--- LLM invoke failed (with suffix): {e} ---")
            try:
                return await llm.ainvoke([HumanMessage(content=text)])
            except Exception as e2:
                print(f"--- LLM invoke fallback also failed: {e2} ---")
                # Do not attempt hosted fallbacks here — keep using the free/local Ollama runner only.
                # Return a deterministic AIMessage-like fallback so callers can continue.
                print("--- Local LLM invocation failed; not attempting hosted fallback per configuration. ---")
                return AIMessage(content=(
                    "I'm temporarily unable to access the local Ollama language model. "
                    "Please try again in a moment or restart the local LLM service."
                ))
    ...
    # If the message was a short greeting/thanks/goodbye, return deterministic polite reply now
    if early_response:
        try:
            final_response = early_response
            final_response_readme = "# Assistant Response\n\n" + final_response + "\n\n_Generated by Bariatric-GPT_"

            memory_summary = prev_memory if prev_memory else ""

            try:
                new_log_obj = {"recent_user_prompts": [last_message], "recent_assistant_responses": [final_response]}
